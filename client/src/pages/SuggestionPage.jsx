...
function SuggestionPage() {
  ...
  return (
    ...
    <Link to={"/home"} className='bg-primaryColor text-white px-5 py-3 rounded-md shadow-lg my-10 ' >Back to home</Link>
    <h3 className=' font-Poppins font-bold mt-10'>Livres recommendés #{suggestion}</h3>
    <div className='grid grid-cols-fluid gap-6'>
      {state.suggestions?.length ? state.suggestions.map(sugg => <SuggestionItem {...sugg} /> ) : null}
      </div>
    </div>
  )
}
